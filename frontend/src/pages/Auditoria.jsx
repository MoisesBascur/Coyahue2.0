...
    // Helper para colores de acción (MODIFICADO)
    const getActionClass = (accion) => {
        if (!accion) return '';
        const act = accion.toLowerCase();
        
        // 🛑 NUEVA REGLA: Creación Masiva (Asignar clase status-masiva)
        if (act.includes('creación masiva')) return 'status-masiva';
        
        if (act.includes('crear') || act.includes('alta') || act.includes('crear usuario')) return 'status-active'; // Verde
        if (act.includes('eliminar') || act.includes('baja')) return 'status-inactive'; // Rojo
        if (act.includes('modificar') || act.includes('editar')) return 'status-warning'; // Azul/Advertencia
        
        return '';
    };
